   ...
   axios(url)
    .then(response  => {
        ...
        $('.card', html).each(async function() {
            ...
            axios(productPageUrl).then(response => {
                const htmlProductPage = response.data
                const pPage = cheerio.load(htmlProductPage)

                let productDescription = pPage('.pb-3', htmlProductPage).find('p').text()
